...
import { VehiclesSearchByVinResponseBody } from 'src/api/controllers/vehicles-controller';
import { createApp } from 'src/api/setup';
import { SearchByVinError } from 'src/domain/errors/search-by-vin.error';
import supertest from 'supertest';

const baseUrl = '/api/v1/vehicles/search';

describe('/api/v1/vehicles/search', () => {
	let app: Express;
	...
	describe('GET /by-vin', () => {
		const resourceUrl = `${baseUrl}/by-vin`;

		it('should respond with 200 Ok - vehicle found', async () => {
			// Arrange
			const vin = '5TEWN72N82Z891171';
			const expectedBody: VehiclesSearchByVinResponseBody = {
		...
		it('should respond with 400 Bad request - invalid vin', async () => {
			...
			// Act
			// Assert
			await supertest(app).get(`${resourceUrl}?vin=${vin}`).expect(400);
		});
